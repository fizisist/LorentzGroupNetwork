...
class InputLinear(nn.Module):
    ...
    """
    def __init__(self, channels_in, channels_out, bias=True, device=torch.device('cpu'), dtype=torch.float):
    ...
    def forward(self, input_scalars, atom_mask, *ignore):
        ...
        !NB: the permute() in this code puts zdim at 0.
        """
...
class InputMPNN(nn.Module):
    ...
    def forward(self, features, atom_mask, edge_features, edge_mask, norms, sq_norms):
        ...
        !NB: again, the last line puts zdim at 0.
        """
        ...
        for mlp, rad_filt, mask in zip(self.mlps, self.rad_filts, self.masks):
            ...
            # TODO: Real-valued GScalar so we don't need any hacks
            # Convert to a form that MaskLevel expects
            # Hack to account for the lack of real-valued GScalar and
